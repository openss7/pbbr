...
static char const ident[] = "src/snmp/ieee8021tcmib.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
#include <getopt.h>
#endif
#include "ieee8021TcMib.h"
#define MASTER 1
#define MY_FACILITY(__pri)	(LOG_DAEMON|(__pri))
#if defined MODULE
#if defined MASTER
const char sa_program[] = "ieee8021TcMib";
...
volatile int ieee8021TcMib_refresh = 1;

/*
 * ieee8021TcMib_variables_oid: object identifier for ieee8021TcMib
 ...
 */
oid ieee8021TcMib_variables_oid[8] = { 1, 3, 111, 2, 802, 1, 1, 1 };
...
/*
 * variable2 ieee8021TcMib_variables: tree for ieee8021TcMib
 * This variable defines function callbacks and type return information for the ieee8021TcMib mib section
 */
struct variable2 ieee8021TcMib_variables[] = {
	/* magic number, variable type, ro/rw, callback fn, L, oidsuffix */

};

/* (L = length of the oidsuffix) */
struct ieee8021TcMib_data *ieee8021TcMibStorage = NULL;

/* global storage of our data, saved in and configured by header_complex() */

void (*ieee8021TcMibold_signal_handler) (int) = NULL;	/* save old signal handler just in case */
void ieee8021TcMib_loop_handler(int);
void ieee8021TcMib_fd_handler(int, void *);

/**
 * @fn void init_ieee8021TcMib(void)
 * @brief ieee8021TcMib initialization routine.
 *
 * This is called when the agent starts up.  At a minimum, registration of the MIB variables
 * structure (ieee8021TcMib_variables) should take place here.  By default the function also
 ...
 */
void
init_ieee8021TcMib(void)
{
	(void) my_fd;
	(void) zeroDotZero_oid;
	(void) snmpTrapOID_oid;
	DEBUGMSGTL(("ieee8021TcMib", "init_ieee8021TcMib: initializing...  "));
	/* register ourselves with the agent to handle our mib tree */
	REGISTER_MIB("ieee8021TcMib", ieee8021TcMib_variables, variable2, ieee8021TcMib_variables_oid);
	snmp_register_callback(SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SHUTDOWN, term_ieee8021TcMib, NULL);
	/* register our config handler(s) to deal with registrations */
	snmpd_register_config_handler("ieee8021TcMib", parse_ieee8021TcMib, NULL, "HELP STRING");
	/* we need to be called back later to store our data */
	snmp_register_callback(SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_ieee8021TcMib, NULL);
	/* place any other initialization junk you need here */
	if (my_readfd >= 0) {
		register_readfd(my_readfd, ieee8021TcMib_fd_handler, (void *) 0);
		register_exceptfd(my_readfd, ieee8021TcMib_fd_handler, (void *) 1);
	}
	ieee8021TcMibold_signal_handler = external_signal_handler[SIGCHLD];
	external_signal_handler[SIGCHLD] = &ieee8021TcMib_loop_handler;
	external_signal_scheduled[SIGCHLD] = 1;
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
}

/**
 * @fn void deinit_ieee8021TcMib(void)
 ...
 */
void
deinit_ieee8021TcMib(void)
{
	DEBUGMSGTL(("ieee8021TcMib", "deinit_ieee8021TcMib: deinitializating...  "));
	external_signal_handler[SIGCHLD] = ieee8021TcMibold_signal_handler;
	if (my_readfd >= 0) {
	...
	unregister_mib(ieee8021TcMib_variables_oid, sizeof(ieee8021TcMib_variables_oid) / sizeof(oid));
	snmpd_unregister_config_handler("ieee8021TcMib");
	/* place any other de-initialization junk you need here */
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
}

int
term_ieee8021TcMib(int majorID, int minorID, void *serverarg, void *clientarg)
{
	DEBUGMSGTL(("ieee8021TcMib", "term_ieee8021TcMib: terminating...  "));
	deinit_ieee8021TcMib();
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
	return 0;
}

/**
 * @fn struct ieee8021TcMib_data *ieee8021TcMib_create(void)
 * @brief create a fresh data structure representing scalars in ieee8021TcMib.
 *
 * Creates a new ieee8021TcMib_data structure by allocating dynamic memory for the structure and
 * initializing the default values of scalars in ieee8021TcMib.
 */
struct ieee8021TcMib_data *
ieee8021TcMib_create(void)
{
	struct ieee8021TcMib_data *StorageNew = SNMP_MALLOC_STRUCT(ieee8021TcMib_data);

	DEBUGMSGTL(("ieee8021TcMib", "ieee8021TcMib_create: creating scalars...  "));
	if (StorageNew != NULL) {
	...
	goto nomem;
      nomem:
	ieee8021TcMib_destroy(&StorageNew);
	goto done;
}

/**
 * @fn struct ieee8021TcMib_data *ieee8021TcMib_duplicate(struct ieee8021TcMib_data *thedata)
 ...
 */
struct ieee8021TcMib_data *
ieee8021TcMib_duplicate(struct ieee8021TcMib_data *thedata)
{
	struct ieee8021TcMib_data *StorageNew = SNMP_MALLOC_STRUCT(ieee8021TcMib_data);

	DEBUGMSGTL(("ieee8021TcMib", "ieee8021TcMib_duplicate: duplicating mib... "));
	if (StorageNew != NULL) {
	}
      done:
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
...
/**
 * @fn int ieee8021TcMib_destroy(struct ieee8021TcMib_data **thedata)
 * @param thedata pointer to the data structure in ieee8021TcMib.
 * @brief delete a scalars structure from ieee8021TcMib.
 *
 * Frees scalars that were previously removed from ieee8021TcMib.  Note that the strings associated
 ...
 */
int
ieee8021TcMib_destroy(struct ieee8021TcMib_data **thedata)
{
	struct ieee8021TcMib_data *StorageDel;

	DEBUGMSGTL(("ieee8021TcMib", "ieee8021TcMib_destroy: deleting scalars...  "));
...
/**
 * @fn int ieee8021TcMib_add(struct ieee8021TcMib_data *thedata)
 * @param thedata the structure representing ieee8021TcMib scalars.
 * @brief adds node to the ieee8021TcMib scalar data set.
 *
 * Adds a scalar structure to the ieee8021TcMib data set.  Note that this function is necessary even
 * when the scalar values are not peristent.
 */
int
ieee8021TcMib_add(struct ieee8021TcMib_data *thedata)
{
	DEBUGMSGTL(("ieee8021TcMib", "ieee8021TcMib_add: adding data...  "));
	if (thedata)
		ieee8021TcMibStorage = thedata;
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
	return SNMPERR_SUCCESS;
}

/**
 * @fn void parse_ieee8021TcMib(const char *token, char *line)
 * @param token token used within the configuration file.
 * @param line line from configuration file matching the token.
 * @brief parse configuration file for ieee8021TcMib entries.
 *
 * This callback is called by UCD-SNMP when it parses a configuration file and finds a configuration
 * file line for the registsred token (in this case ieee8021TcMib).  This routine is invoked by
 * UCD-SNMP to read the values of scalars in the MIB from the configuration file.  Note that this
 * procedure may exist regardless of the persistence of the MIB.  If there are no configured entries
 * in the configuration MIB, this function will simply not be called.
 */
void
parse_ieee8021TcMib(const char *token, char *line)
{
	size_t tmpsize;
	struct ieee8021TcMib_data *StorageTmp = ieee8021TcMib_create();

	DEBUGMSGTL(("ieee8021TcMib", "parse_ieee8021TcMib: parsing config...  "));
	if (StorageTmp == NULL) {
		config_perror("malloc failure");
		return;
	}
	/* XXX: remove individual scalars that are not persistent */
	ieee8021TcMib_add(StorageTmp);
	(void) tmpsize;
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
}

/*
 * store_ieee8021TcMib(): stores .conf file entries needed to configure the mib.
 */
int
store_ieee8021TcMib(int majorID, int minorID, void *serverarg, void *clientarg)
{
	char line[SNMP_MAXBUF];
	char *cptr;
	size_t tmpsize;
	struct ieee8021TcMib_data *StorageTmp;

	DEBUGMSGTL(("ieee8021TcMib", "store_ieee8021TcMib: storing data...  "));
	refresh_ieee8021TcMib(1);
	if ((StorageTmp = ieee8021TcMibStorage) == NULL) {
		DEBUGMSGTL(("ieee8021TcMib", "error.\n"));
...
/**
 * @fn int check_ieee8021TcMib(struct ieee8021TcMib_data *StorageTmp, struct ieee8021TcMib_data *StorageOld)
 ...
 */
int
check_ieee8021TcMib(struct ieee8021TcMib_data *StorageTmp, struct ieee8021TcMib_data *StorageOld)
{
...
/**
 * @fn int update_ieee8021TcMib(struct ieee8021TcMib_data *StorageTmp, struct ieee8021TcMib_data *StorageOld)
 * @param StorageTmp the data as updated.
 * @param StorageOld the data previous to update.
 ...
 * consistency already checked (see check_ieee8021TcMib()).  This function can
 ...
 */
int
update_ieee8021TcMib(struct ieee8021TcMib_data *StorageTmp, struct ieee8021TcMib_data *StorageOld)
{
	/* XXX: provide code to update the row with the underlying device */
	ieee8021TcMib_refresh = 1;
	return SNMP_ERR_NOERROR;
}

/**
 * @fn revert_ieee8021TcMib(struct 
 * @fn void revert_ieee8021TcMib(struct ieee8021TcMib_data *StorageTmp, struct ieee8021TcMib_data *StorageOld)
 * @param StorageTmp the data as updated.
 * @param StorageOld the data previous to update.
 * @brief perform the undo operation (UNDO phase)
 */
void
revert_ieee8021TcMib(struct ieee8021TcMib_data *StorageTmp, struct ieee8021TcMib_data *StorageOld)
{
	/* XXX: provide code to revert the row with the underlying device */
	update_ieee8021TcMib(StorageOld, NULL);
}

/**
 * @fn void refresh_ieee8021TcMib(int force)
 * @param force forced refresh when non-zero.
 * @brief refresh the scalar values of ieee8021TcMib.
 *
 ...
 */
void
refresh_ieee8021TcMib(int force)
{
	if (ieee8021TcMibStorage == NULL) {
		struct ieee8021TcMib_data *StorageNew;

		if ((StorageNew = ieee8021TcMib_create()) == NULL)
			return;
		ieee8021TcMibStorage = StorageNew;
		ieee8021TcMib_refresh = 1;
	}
	if (!force && ieee8021TcMib_refresh == 0)
		return;
	DEBUGMSGTL(("ieee8021TcMib", "refresh_ieee8021TcMib: refreshing...  "));
	/* XXX: Update scalars as required here... */
	ieee8021TcMib_refresh = 0;
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
}

/**
 * @fn u_char * var_ieee8021TcMib(struct variable *vp, oid *name, size_t *length, int exact, size_t *var_len, WriteMethod **write_method)
 ...
 * @brief locate variables in ieee8021TcMib.
 *
 ...
 */
u_char *
var_ieee8021TcMib(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	struct ieee8021TcMib_data *StorageTmp;
	u_char *rval;

	DEBUGMSGTL(("ieee8021TcMib", "var_ieee8021TcMib: lookup up varbind...  "));
	if (header_generic(vp, name, length, exact, var_len, write_method) == MATCH_FAILED)
		return NULL;
	/* Refresh the MIB values if required. */
	refresh_ieee8021TcMib(0);
	if ((StorageTmp = ieee8021TcMibStorage) == NULL) {
		DEBUGMSGTL(("ieee8021TcMib", "no datastructure.\n"));
		return NULL;
	}
	*write_method = NULL;
	*var_len = 0;
	...
	if (rval)
		DEBUGMSGTL(("ieee8021TcMib", "found.\n"));
	else
		DEBUGMSGTL(("ieee8021TcMib", "not found.\n"));
	return (rval);
}

/**
 * @fn void ieee8021TcMib_loop_handler(int dummy)
 ...
 */
void
ieee8021TcMib_loop_handler(int sig)
{
	DEBUGMSGTL(("ieee8021TcMib", "ieee8021TcMib_loop_handler: executing loop handler...  "));
	...
	if (ieee8021TcMibold_signal_handler != NULL)
		(*ieee8021TcMibold_signal_handler) (sig);
	DEBUGMSGTL(("ieee8021TcMib", "done.\n"));
}

/**
 * @fn void ieee8021TcMib_fd_handler(int fd, void *dummy)
 ...
 */
void
ieee8021TcMib_fd_handler(int fd, void *dummy)
{
	DEBUGMSGTL(("ieee8021TcMib", "ieee8021TcMib_fd_handler: executing fd handler...  "));
